...
function PhotoViewer() {
    ...
    const buildTitle = () => ['Autor: ', currentImage.imageDetail.author, ', Camera Model: ', currentImage.imageDetail.camera].join('');

    return (
        <div>
            {isOpened && (
                <Lightbox
                    imageTitle={!currentImage.fetching ? buildTitle() : ''}
                    ...
                    toolbarButtons={[
                        <WhatsappShareButton url={currentImage.imageDetail.full_picture} children={<WhatsappIcon size={32} round={true} />} />
                    ]}
                />
            )}
        </div>
